            ...
            for fi = 1:length(tx_files)
                ...
                for b = 1:numBlocks_img
                    ...
                    if mod(b-1, insert_period) == 0 && coord_cnt < m
                        ...
                        rx_coord_mrc = rx_coord_mrc + conj(h1) * rx_coord_sig;
                       
                    end
                end
                
                %% =============================
                rx_coord_mrc =rx_coord_mrc/sum(abs(h).^2);
                var_eff = 10/(10^(SNR/10));
                rx_coord_llr = qamdemod( ...
                    rx_coord_mrc, ...
                    16, ...
                    'UnitAveragePower', false, ...
                    'OutputType', 'llr', ...
                    'NoiseVariance', var_eff);
                
                dec1 = ldpcDecode(rx_coord_llr, cfgDec1, maxnumiter);
                rx_coord_bits = dec1(1:coord_len);
