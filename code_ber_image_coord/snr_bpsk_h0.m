...
for si = 1:length(SNR_list)
    ...
    for b = 1:numBlocks_img
        ...
        h0=h0+1;
        if h0>1000000,fprintf("h0→ %d\n,mc→ %d\n", h0,mc);end
        if h0>1000000,stopAll = true;end
        
        rx_img_sig = awgn(rx_img_sig, SNR, 'measured','dB');
        
        var = 1/(10^(SNR/10));
        rx_coord_llr = bpskdemod(rx_coord_sig, h, var, 'llr');
        rx_img_llr   = bpskdemod(rx_img_sig,   h, var, 'llr');
    ...
    if stopAll
        fprintf("Stop at mc = %d, h = %d\n", mc, h);
        MC = mc;
        break;              % 跳出 mc（mc 是最后一次）
    end

    end
    fprintf("ber at MC = %d, h0 = %d\n", MC, h0);% mc=42,h0=1001826
